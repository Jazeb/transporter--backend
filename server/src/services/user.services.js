...
function getOrder(order_id) {
    return new Promise((resolve, reject) => {
        const include = [{ model: Vendors }];
        ServiceOrders.findOne({ where: { id: order_id }, include })
...
function updateVendors(data) {
    return new Promise((resolve, reject) => {
        Vendors.update(data, { where: { id: data.id }, attributes: { exclude: ['id', 'email', 'password', 'created_at', 'updated_at'] } })
            .then(_ => Vendors.findOne({ where: { id: data.id } }).then(vendor => resolve(vendor)))
            .catch(err => reject(err));
    });
}
...
function addVehicle(data, id) {
    return new Promise((resolve, reject) => {
        Vendors.update(data, { where: { id } }).then(_ => resolve(true)).catch(err => reject(err))
    });
}

function addContactUs(body) {
    return new Promise((resolve, reject) => {
